...
export default function QuestionPage(){
 ...
 return <main className="connect-page"><section className="connect-card"><header className="connect-header"><span>{en?"2. Ask a question":"2. 질문하기"}</span><Link href="/">{en?"Close":"닫기"}</Link></header><h1>{en?"Leave a question.":"질문을 남겨주세요."}</h1><textarea value={text} onChange={e=>setText(e.target.value)} autoFocus/><button className="connect-next" disabled={!text.trim()||status==="sending"} onClick={submit}>{status==="sending"?(en?"Sending…":"보내는 중…"):(en?"Send question":"질문 보내기")}</button>{status==="error"&&<p className="connect-error">{en?"Could not send. Please try again.":"전송하지 못했어요. 다시 시도해주세요."}</p>}</section></main>;
}
