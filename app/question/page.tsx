...
export default function QuestionPage(){
 ...
 useEffect(()=>{let alive=true;const refresh=()=>readState().then(s=>{if(alive){setEnabled(s.questions_enabled);setLocale(s.locale);}}).catch(()=>undefined);void refresh();const off=subscribeTables(refresh,["exhibit_state"]);const backup=setInterval(refresh,15000);return()=>{alive=false;off();clearInterval(backup);};},[]);
