...
export default function GalleryPage(){
 const state=useExhibitState(),{display_mode:liveMode,questions_enabled:questionsEnabled,locale}=state,tr=locale==='tr',en=locale==='en';
 ...
 function admin(){let key=sessionStorage.getItem('ma-admin')??'';if(!key)key=prompt(tr?"Yönetici şifresini girin.":en?'Enter administrator password.':'관리자 암호를 입력하세요.')?.trim()??'';return key;}
 function report(error:{code?:string;message?:string}){if(error.code==='42501'){sessionStorage.removeItem('ma-admin');setNotice(tr?"Yönetici şifresi yanlış.":en?'Incorrect administrator password.':'관리자 암호가 맞지 않습니다.');}else setNotice(tr?"Bağlantı kesildi. Mevcut ekran korundu. Lütfen tekrar deneyin.":en?'Connection interrupted. The current screen is preserved. Please try again.':'연결이 원활하지 않습니다. 현재 화면은 유지됩니다. 다시 눌러주세요.');}
 ...
 async function finish(){const d=drag.current;if(!d)return;drag.current=null;const key=admin();if(!key){movingIds.current.delete(d.id);void load();return;}sessionStorage.setItem('ma-admin',key);pendingMoves.current.set(d.id,{...d});try{sessionStorage.setItem('ma-pending-moves',JSON.stringify([...pendingMoves.current]));}catch{}await saveMoves();if(pendingMoves.current.has(d.id))setNotice(tr?"Konum bu cihaza kaydedildi; eşitleme bekleniyor.":en?'Position saved on this device; waiting to sync.':'위치를 기기에 보관했습니다. 연결되면 다시 저장합니다.');}
 async function remove(kind:Kind,id:string){if(ending.current)return;if(!confirm(tr?"Bu öğe silinsin mi?":en?'Delete this item?':'삭제할까요?'))return;const key=admin();if(!key)return;
 ...
 return <main className="gallery-page exhibit-page"><header className="gallery-header exhibit-header"><div className="gallery-title"><h1>{tr?"Mobil Arkeoloji":en?'Mobile Archaeology':'모바일 고고학'}</h1><p>{tr?"Kaydetmek için fotoğrafa basılı tutun.":en?'Press and hold a photo to save it.':'사진을 길게 눌러 선택하고 저장하세요.'}</p></div><Link className="gallery-back" href="/">{tr?"Geri":en?'Back':'돌아가기'}</Link><div className="exhibit-controls"><div className="language-switch" aria-label="워크숍 언어"><button aria-pressed={locale==='ko'} className={locale==='ko'?'active':''} disabled={busy} onClick={()=>setState({locale:'ko'})}>한국어</button><button aria-pressed={locale==='tr'} className={locale==='tr'?'active':''} disabled={busy} onClick={()=>setState({locale:'tr'})}>Türkçe</button><button aria-pressed={locale==='en'} className={locale==='en'?'active':''} disabled={busy} onClick={()=>setState({locale:'en'})}>English</button></div><div className="exhibit-modes"><button className={mode==='photos'?'active':''} disabled={busy} onClick={()=>setState({mode:mode==='photos'?'none':'photos'})}>{tr?"1. Mobil Arkeoloji":en?'1. Mobile Archaeology':'1. 모바일 고고학'}</button><button className={questionsEnabled?'active':''} disabled={busy} onClick={()=>setState({questions:!questionsEnabled})}>{tr?"2. Sorular":en?'2. Questions':'2. 질문'}</button><button className={mode==='play'?'active':''} disabled={busy} onClick={()=>setState({mode:mode==='play'?'none':'play'})}>{tr?"3. İzlerin Oyunu":en?'3. Play of Traces':'3. 기척의 놀이'}</button><EndWorkshop locale={locale} capture={capture} onBusy={value=>{ending.current=value;setBusy(value);}} onFinished={()=>{setCapturing(false);void load();}}/></div></div><button className="gallery-qr" onClick={()=>setQr(true)}><QRCodeSVG value={MOBILE_URL}/></button></header>
