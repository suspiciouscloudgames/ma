...
const participants = Array.from({ length: 20 }, () => crypto.randomUUID());
...
console.log(JSON.stringify({ questionWrites: `${writes.filter(v=>v.ok).length}/20`, readRequests: `${reads.filter(Boolean).length}/80`, cleanup: `${cleanup.filter(Boolean).length}/${ids.length}`, reset: reset.ok, elapsedMs: Date.now()-started }, null, 2));
