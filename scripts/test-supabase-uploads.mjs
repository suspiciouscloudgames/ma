...
const cleanupFiles=await Promise.all(results.flatMap(r=>r.paths).map(path=>fetch(`${url}/storage/v1/object/workshop-photos/${path}`,{method:"DELETE",headers:{...auth,"x-admin-key":"0000"}})));
console.log(JSON.stringify({uploads:`${results.filter(r=>r.ok).length}/${count}`,metadataCleanup:`${cleanupRows.filter(r=>r.ok).length}/${cleanupRows.length}`,fileCleanup:`${cleanupFiles.filter(r=>r.ok).length}/${cleanupFiles.length}`,elapsedMs:Date.now()-started},null,2));
